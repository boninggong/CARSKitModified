...
public class DataDAO {
    ...
    /**
     * @param innerIds
     *            inner user ids as list of int
     * @return raw item ids as list of strings
     */
    public String[] getItemId(List<Integer> innerIds) {
        String[] res = new String[innerIds.size()];
        if (idItems == null)
            idItems = itemIds.inverse();

        for(int i = 0 ; i < innerIds.size() ; i++){
            int innerId = innerIds.get(i);
            res[i] = idItems.get(innerId);
        }
