...
public abstract class Recommender implements Runnable{
    ...
    protected Map<Measure, Double> evalRankings() throws Exception {
        ...
        if (isResultsOut) {
            preds = new ArrayList<String>(1500);
            String header = numRecs + "\nuserId,correctItems,contexts,";
            for(int i = 1; i < numRecs+1; i++) {
                header += "p" + i + ",";
            }
            preds.add(header.substring(0, header.length() - 1));
            toFile = workingPath
                    + String.format("%s-%s.csv", algoName, foldInfo); // the output-file name
            FileIO.deleteFile(toFile); // delete possibly old files
        }
        ...
        for (int u:uciList.keySet()) {
            ...
            for (int c : cis.keySet()) {
                ...
                String[] posItemsString = new String[posItems.size()];
                int countr = 0;

                // intersect with the candidate items
                for (Integer j : posItems) {
                    posItemsString[countr] = rateDao.getItemId(j);
                    countr++;
                ...
                for (Map.Entry<Integer, Double> kv : recomd) {
                    ...
                    if (isResultsOut && count < numTopNRanks) {
                        ...
                        sb.append(rateDao.getItemId(item));

                        if (posItems.contains(item))
                            sb.append(""); // indicating correct recommendation

                        sb.append(";").append(kv.getValue().floatValue());
                ...
                String corItems = "";
                for (Integer corr: correctItems) {
                    corItems += rateDao.getItemId(corr) + ";";
                }
                corItems = corItems.substring(0, corItems.length() - 1) + ",";

                // output predictions
                if (isResultsOut) {
                    // restore back to the original user id
                    preds.add(rateDao.getUserId(uu) + "," + corItems + rateDao.getContextSituationFromInnerId(c) + "," + sb.toString());
                    if (preds.size() >= 1000) {
                        FileIO.writeList(toFile, preds, true);
                        preds.clear();
                    }
